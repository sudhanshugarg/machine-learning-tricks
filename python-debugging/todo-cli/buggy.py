...
def save_tasks(tasks, path=DATA_FILE):
    # Bug: writer receives potentially stale list in some command paths.
...
def add_task(title, priority, due, path=DATA_FILE):
    ...
    # Bug: allows out-of-range and non-int priorities if they can be cast.
    try:
...
def delete_task(task_id, path=DATA_FILE):
    ...
    # Bug: deletes by title, which may remove extra tasks.
    new_tasks = [task for task in tasks if task["title"] != target["title"]]

    # Bug: reindexes IDs after delete, causing ID instability.
...
def build_parser():
    ...
    add.add_argument("--priority", required=True)
